	...
	<div class="grid grid-cols-1 gap-4 mx-auto max-w-7xl md:gap-8 sm:grid-cols-4 lg:flex lg:gap-16 xl:gap-24"><?php
		...
		foreach ($site->footer()->toBlocks() as $footer) : ?>
          ...
          $svg = $target = '';
		        ...
		        $svg = '&nbsp;<svg class="fill-white opacity-80 inline mb-[2px] h-3 w-3"><use xlink:href="#external"></use></svg>';
        ...
        <?php if ($site->address()->isNotEmpty()) : ?>
            <div class="col-span-1 text-white lg:text-right sm:col-span-4 lg:flex-grow address">
                ...
                <?php e($site->addressphone()->isNotEmpty(), '<a title="' . t('site.footer.call') . '" href="tel:' . cleanTel($site->addressphone()) . '" class="inline-block mb-1 text-sm"><svg class="fill-white inline mb-[2px] mr-[8px] h-3 w-3"><use xlink:href="#phone"></use></svg><span class="border-b border-white border-dotted opacity-80">' . $site->addressphone() . '</span></a><br>') ?>
                <?php e($site->addressemail()->isNotEmpty(), '<a title="' . t('site.footer.mailto') . '" href="mailto:' . $site->addressemail() . '" class="inline-block text-sm"><svg class="fill-white inline mb-[2px] mr-[8px] h-3 w-3"><use xlink:href="#mailto"></use></svg><span class="border-b border-white border-dotted opacity-80">' . $site->addressemail() . '</span></a>') ?>
                ...
                if ($site->facebookurl()->isNotEmpty()  || $site->linkedinurl()->isNotEmpty() || $site->instagramurl()->isNotEmpty() || $site->youtubeurl()->isNotEmpty()) : ?>
                    <div class="flex gap-2 mt-4 lg:justify-end">
                        ...
                        if ($site->facebookurl()->isNotEmpty()) :
                            ...
                            echo '<svg class="w-5 h-5 opacity-80 fill-white"><use xlink:href="#facebook"></use></svg>';
                            echo '</a>';
                        endif;
                        ...
                        if ($site->linkedinurl()->isNotEmpty()) :
                            ...
                            echo '<svg class="w-5 h-5 opacity-80 fill-white"><use xlink:href="#linkedin"></use></svg>';
                            echo '</a>';
                        endif;
                        ...
                        if ($site->instagramurl()->isNotEmpty()) :
                            ...
                            echo '<svg class="w-5 h-5 opacity-80 fill-white"><use xlink:href="#instagram"></use></svg>';
                            echo '</a>';
                        endif;
                        ...
                        if ($site->youtubeurl()->isNotEmpty()) :
                            ...
                            echo '<svg class="w-5 h-5 opacity-80 fill-white"><use xlink:href="#youtube"></use></svg>';
                            echo '</a>';
                        endif;?>
...
<?php
// Nav SVG sprite — project patch overrides plugin default
...
// Project-level SVG definitions (legacy)
